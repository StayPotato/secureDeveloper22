...
> sqli 방지를 위한 바인딩 
 ...
 main.go:596
> pw 보호를 위한 단방향 해시 추가
> 이를 위해 seed.sql pw에 대하여 해시 함수를 이용한 뒤, 제작할 수 있도록 수정함

 main.go:606
> 해시 검증 함수 추가
> 해시된 pw를 검증할 수 있도록 해시-평문을 비교하는 함수 추가

 main.go:618
> username, phone, email에 대한 중복검사 루틴 추가
> 작동에 오류가 있어 주석처리함
> Query()를 사용하여 username, phone, email 중 하나라도 중복되는 id가 있으면 혼란을 방지하기 위해 회원가입을 할 수 없도록 조치하려고 함
> 그러나 어떠한 사유로 제대로 검증되지 않아 사용하지 못함. (추측으로는 SQL 구문 오류인 것 같으나, 직접 sqlite3를 사용하여 명령어를 입력했을 시 정상적으로 id값 혹은 err가 출력되는 것을 확인하긴 함)

 main.go:643
> 게시글 목록 탐색 기능 추가
> QueryRow()를 사용해 모든 posts 테이블 값을 출력하도록 제작하려고 함.
> 618라인과 동일한 이유로 검색이 명확히 진행되지 않음.

 main.go:635
> 게시글 작성 기능 추가
> 작성된 게시글의 시간이 정상적으로 입력될 수 있도록 time.Now().Format(time.RFC3339)을 사용함.
-----
main을 분리하진 못했으나, 분리할 수 있다면 회원가입-로그인/ 게시판 업로드-조회/ 금융 예금 및 출금으로 분리하여 작업할 거 같음.
그 까닭으론 서로 사용하는 table단위로 분리하는 것이 관리하기 쉬울 것 같은 것이 1 까닭.